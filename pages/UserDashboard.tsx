            ...
            {supportInfo && (
                 <div className="bg-base-100 p-6 rounded-2xl border border-white/5">
                     ...
                     <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
                         {supportInfo.whatsapp && <a href={`https://wa.me/${supportInfo.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors"><WhatsAppIcon className="w-5 h-5 text-green-400"/> WhatsApp</a>}
                         {supportInfo.telegram && <a href={`https://t.me/${supportInfo.telegram}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors"><TelegramIcon className="w-5 h-5 text-sky-400"/> Telegram</a>}
                         {supportInfo.instagram && <a href={`https://instagram.com/${supportInfo.instagram}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors"><InstagramIcon className="w-5 h-5 text-pink-400"/> Instagram</a>}
                         {supportInfo.youtube && <a href={supportInfo.youtube} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors"><YouTubeIcon className="w-5 h-5 text-red-500"/> YouTube</a>}
                         {supportInfo.email && <a href={`mailto:${supportInfo.email}`} className="flex items-center gap-2 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors"><EnvelopeIcon className="w-5 h-5 text-slate-400"/> Email</a>}
                         {supportInfo.mobile && <a href={`tel:${supportInfo.mobile}`} className="flex items-center gap-2 p-3 rounded-lg bg-white/5 hover:bg-white/10 transition-colors"><PhoneIcon className="w-5 h-5 text-slate-400"/> Call Us</a>}
                     </div>
                 </div>
            )}
