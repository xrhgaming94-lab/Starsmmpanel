...
import { 
    ...
    updateMockServiceInStorage, addMockServiceToStorage
...
const AdminDashboard: React.FC<{ user: User; onLogout: () => void; theme: string; onToggleTheme: () => void; }> = ({ user, onLogout, theme, onToggleTheme }) => {
    ...
    const fetchData = useCallback(async () => {
        setLoading(true);
        try {
            const [fetchedUsers, fetchedOrders, fetchedServices, fetchedDeposits, fetchedCategories, fetchedCoupons, fetchedMaintenance] = await Promise.all([
                getAllUsers(), 
                getOrders(), 
                getServicePackages(), 
                getDepositRequests(), 
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <div className="animate-fade-in">
                            ...
                            {activeTab === 'settings' && <SettingsContent maintenanceStatus={maintenanceStatus} categories={categories} onDeleteCategory={initiateDeleteCategory} onDataChange={fetchData} isMockMode={isMockMode} setShowPermissionModal={setShowPermissionModal} />}
                        </div>
                    </>
                )}
            </main>
...
const SettingsContent: React.FC<{maintenanceStatus: MaintenanceStatus|null, categories: Category[], onDeleteCategory: (id: string, name: string) => void, onDataChange: () => void, isMockMode: boolean, setShowPermissionModal: (b: boolean) => void}> = ({maintenanceStatus, categories, onDeleteCategory, onDataChange, isMockMode, setShowPermissionModal}) => {
    ...
    useEffect(() => {
        const loadConfigs = async () => {
            ...
            try {
                const [tg, sec, email] = await Promise.all([getTelegramConfig(), getAdminSecurity(), getEmailConfig()]);
                if (tg) { setTgBotToken(tg.botToken); setTgChannelId(tg.channelId); setTgOwnerId(tg.ownerChatId); }
                if (sec) setPin(sec.pin);
    ...
    const saveTelegramConfig = async () => {
        if(isMockMode) return alert("Cannot save Telegram config in mock mode");
        try { await updateTelegramConfig({ botToken: tgBotToken, channelId: tgChannelId, ownerChatId: tgOwnerId }); alert("Telegram Config Saved!"); } catch(e:any) { if(e.code==='permission-denied') setShowPermissionModal(true); else alert(e.message); }
    };
    
    const saveEmailConfig = async () => {
        if(isMockMode) return alert("Cannot save Email config in mock mode");
        try { await updateEmailConfig({ serviceId: emailServiceId, templateId: emailTemplateId, publicKey: emailPublicKey }); alert("Email Config Saved!"); } catch(e:any) { if(e.code==='permission-denied') setShowPermissionModal(true); else alert(e.message); }
    };

    const savePin = async () => {
        if (pin.length < 4) return alert("PIN must be at least 4 digits");
        try { if(isMockMode) { saveMockAdminSecurityToStorage(pin); } else { await updateAdminSecurity(pin); } alert("Security PIN Updated!"); } catch(e:any) { if(e.code==='permission-denied') setShowPermissionModal(true); else alert(e.message); }
    };
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-6">
                ...
                <div className="bg-base-100 p-6 rounded-2xl border border-white/5">
                    ...
                    <div className="flex gap-2 items-end">
                        <div className="flex-1"><label className="text-[10px] font-bold text-slate-500 uppercase">Access PIN</label><input type={showPin ? "text" : "password"} value={pin} onChange={e => setPin(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white" placeholder="Enter PIN" /></div>
                ...
                <div className="bg-base-100 p-6 rounded-2xl border border-white/5">
                    ...
                    <form onSubmit={handleAddCategory} className="mb-4 space-y-3">
                        <div className="flex gap-2">
                            <input type="text" value={catName} onChange={e => setCatName(e.target.value)} placeholder="New Category Name" className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:border-secondary outline-none"/>
                            <button type="submit" className="bg-white/10 text-white px-4 py-2 rounded-xl text-xs font-bold uppercase hover:bg-white/20">Add</button>
                        </div>
                        <div>
                            <label className="block text-[10px] font-bold text-slate-500 uppercase mb-2">Select Icon</label>
                            <div className="grid grid-cols-7 gap-2">
                                {Object.entries(categoryIcons).map(([name, IconComponent]) => (
                                    <button
                                        key={name}
                                        type="button"
                                        onClick={() => setCatIcon(name)}
                                        className={`p-2 rounded-lg flex items-center justify-center transition-all ${catIcon === name ? 'bg-secondary text-white shadow-lg scale-110' : 'bg-white/5 text-slate-400 hover:bg-white/10 hover:text-white'}`}
                                        title={name}
                                    >
                    ...
                    <div className="flex flex-wrap gap-2 mt-4">{categories.map(c => {
                        const Icon = iconMap[c.iconName] || HeartIcon;
                        return (
                            <div key={c.id} className="flex items-center gap-2 px-3 py-1.5 bg-white/5 rounded-lg border border-white/5">
                                <Icon className="w-3 h-3 text-slate-400"/>
                                <span className="text-xs text-slate-300">{c.name}</span>
                                <button onClick={() => onDeleteCategory(c.id, c.name)} className="text-red-400 hover:text-red-300 ml-1"><TrashIcon className="w-3 h-3"/></button>
                            </div>
                        );
                    })}</div>
                </div>
            ...
            <div className="space-y-6">
                <div className="bg-base-100 p-6 rounded-2xl border border-white/5">
                    <h3 className="font-bold text-white mb-4 flex items-center gap-2"><PaperAirplaneIcon className="w-5 h-5 text-blue-400"/> Telegram Config</h3>
                    <div className="space-y-3">
                        <InputField label="Bot Token" name="botToken" value={tgBotToken} onChange={e => setTgBotToken(e.target.value)} />
                        <InputField label="Channel ID" name="channelId" value={tgChannelId} onChange={e => setTgChannelId(e.target.value)} />
                        <InputField label="Owner ID" name="ownerId" value={tgOwnerId} onChange={e => setTgOwnerId(e.target.value)} />
                        <button onClick={saveTelegramConfig} className="w-full py-2 bg-blue-600/20 text-blue-400 font-bold rounded-xl text-xs uppercase hover:bg-blue-600/40">Save Telegram Settings</button>
                    </div>
                </div>
                <div className="bg-base-100 p-6 rounded-2xl border border-white/5">
